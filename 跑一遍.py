# -*- coding: utf-8 -*-
"""
Created on Sun Jan 10 22:03:23 2021
...
if __name__=="__main__":
    ...
    features = kmeans_model(raw_train_features,600)
    labels = np.reshape(np.array(label_list), (-1,))
    X_train, X_test, y_train, y_test = train_test_split(features, labels, test_size=0.2, random_state=42)
    print('the dimensions of X_train and y_train',X_train.shape,y_train.shape)
    print('the dimensions of X_test and y_train',X_test.shape,y_test.shape)
    # build svm model
    print('———————————————————————————————————————build svm model—————————————————————————————————————')
    svm_model = LinearSVC()
    svm_model.fit(X_train, y_train)
    predict = svm_model.predict(X_test)
    svm_accuracy_value = accuracy_score(y_test, predict)
    print('svm accuracy is :', svm_accuracy_value)
    cp = sm.classification_report(y_test, predict)
    print(cp)
